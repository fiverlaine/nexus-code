...
import { trackCustomEvent, trackEvent } from './utils/facebookPixel';
...
function App() {
  ...
  const handleBrokerClick = async () => {
    // Rastrear evento personalizado "Acessou betlion" no Facebook Pixel
    await trackCustomEvent('Acessou betlion', {
      source: 'nexus_code',
      broker_url: 'https://betlionpro.com',
      aff_id: '156168',
      utm_source: 'code',
      src: 'code'
    });
  ...
  // Rastrear PageView quando o componente carregar (apenas se não estiver em iframe para evitar duplicação)
  useEffect(() => {
    const isInIframe = window.self !== window.top;
    
    // Se não estiver em iframe, rastrear PageView com parâmetros completos
    // Se estiver em iframe, o PageView já foi rastreado pelo parent
    if (!isInIframe) {
      trackEvent('PageView').catch(err => {
        console.warn('Erro ao rastrear PageView:', err);
      });
    }
  }, []);
